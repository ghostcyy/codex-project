...
- **Edit mode** (`assets/edit-mode.js`) — **E** key toggles inline text editing for non-coders; edits auto-saved to `localStorage`; **Ctrl+S** or toolbar button exports a clean standalone `.html` with all changes baked in
...
2. **Style / theme.** Which of the 36 themes fits? If unsure, recommend 2-3
   ...
   - Edgy / cyber / launch → `cyberpunk-neon`, `vaporwave`, `y2k-chrome`,
     `neo-brutalism`
   When you generate the deck, set the user's chosen theme as the default
   `theme-link`, then include 3-5 additional fitting backup themes in
   `data-themes` so the deck can still cycle with `T` during rehearsal.
...
1. **Scaffold a new deck.** From the repo root:
   ...
   ```
   If `assets/edit-mode.js` is included, press `E` to edit text inline and
   `Ctrl+S` to download a clean standalone `.html` with the changes baked in.
2. **Pick a theme.** Open the deck and press `T` to cycle. Or hard-code it:
   ```html
   <link rel="stylesheet" id="theme-link" href="../assets/themes/aurora.css">
   ```
   Prefer this pattern in generated decks:
   ```html
   <html data-themes="retro-tv,academic-paper,news-broadcast,corporate-clean">
   <link rel="stylesheet" id="theme-link" href="../assets/themes/retro-tv.css">
   ```
   Default should be the user-requested theme. The rest are backup themes that
   fit the same content and can be toggled with `T`.
...
│   ├── edit-mode.js         (E key inline editor — include after runtime.js)
...
E                                       toggle Edit Mode — click any text to edit inline
                                          (toolbar: Bold / Italic / Underline / Undo / Download)
Ctrl+S                                  download the edited deck as a standalone .html
