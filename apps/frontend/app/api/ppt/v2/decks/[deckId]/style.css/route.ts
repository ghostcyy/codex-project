...
export async function GET(_request: Request, context: { params: Promise<{ deckId: string }> }) {
  ...
  const response = await fetch(`${API_BASE_URL}/ppt/v2/decks/${encodeURIComponent(deckId)}/style.css`, {
  ...
  return new Response(response.body, {
    ...
    headers: {
      "Content-Type": response.headers.get("Content-Type") ?? "text/css; charset=utf-8",
