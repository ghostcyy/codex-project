...
export async function GET(_request: Request, context: { params: Promise<{ deckId: string }> }) {
  ...
  return proxyDeckFile(deckId, "verification-report.json", "application/json; charset=utf-8");
}

async function proxyDeckFile(deckId: string, file: string, fallbackType: string) {
  const token = (await cookies()).get(SESSION_COOKIE_NAME)?.value;
  if (!token) {
    return NextResponse.json({ message: "Unauthorized" }, { status: 401 });
  }
  const response = await fetch(`${API_BASE_URL}/ppt/v2/decks/${encodeURIComponent(deckId)}/${file}`, {
