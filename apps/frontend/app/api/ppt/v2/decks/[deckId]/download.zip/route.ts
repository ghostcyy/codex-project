...
export async function GET(_request: Request, context: { params: Promise<{ deckId: string }> }) {
  ...
  const response = await fetch(`${API_BASE_URL}/ppt/v2/decks/${encodeURIComponent(deckId)}/download.zip`, {
  ...
  return new Response(response.body, {
    ...
    headers: {
      ...
      "Content-Disposition": response.headers.get("Content-Disposition") ?? 'attachment; filename="html-ppt-deck.zip"'
    }
  });
}
