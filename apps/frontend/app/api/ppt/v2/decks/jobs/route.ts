...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${API_BASE_URL}/ppt/v2/decks/jobs`, {
    method: "POST",
    headers: {
...
export async function GET() {
  ...
  return NextResponse.json(
    {
      message: response.ok ? "HTML-PPT v2 jobs returned a non-JSON response." : `Backend returned ${response.status}.`,
      detail: text.slice(0, 2000)
    },
    { status: response.status }
  );
